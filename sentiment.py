...
def predict_sentiment(content: str) -> int:
    ...
    if logits.shape[-1] == 1:
        ...
        if config_path.exists():
            ...
            thresholds = model_config.get("thresholds", thresholds)

        score = float(logits.squeeze(-1).detach().cpu().item())
        if score < float(thresholds[0]):
            return 0
        if score < float(thresholds[1]):
            return 1
        if score < float(thresholds[2]):
            return 2
        return 3
